...
export class CompaniesService implements CoreService<ICompany> {
  ...
  getOne(idCompany: string): Promise<ICompany> {
    return API.get(`${CompaniesService.uri}/${idCompany}`, {}).then((result: any) => result.data as ICompany)
  }

  createOrUpdateOne(company: ICompany): Promise<ICompany> {
    if (company._id) {
      return API.put(`${CompaniesService.uri}/${company._id}`, company).then((result: any) => result.data as ICompany)
    }
    return API.post(`${CompaniesService.uri}`, company).then((result: any) => result.data as ICompany)
  }
