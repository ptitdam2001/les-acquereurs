...
export const CompanyForm: React.FC<CompanyFormProps> = (props: CompanyFormProps) => {
	...
	const formik = useFormik({
		...
		onSubmit: (values: any) => {
			...
			dispatch(createOrUpdate((values as ICompany)))
			onSave && onSave()
		},
	})
	...
	return (
		<Box className={classes.root}>
			<form onSubmit={formik.handleSubmit} className={classes.form}>
				...
				<AddressFormGroup address={formik.values.address} onChange={(addr: IAddress) => {
					formik.values.address = addr
					formik.handleChange('address')
				}} />

				<FormControlLabel
					control={<Switch checked={active} onChange={handleChangeActive} name="active" />}
					label="Active"
				/>
