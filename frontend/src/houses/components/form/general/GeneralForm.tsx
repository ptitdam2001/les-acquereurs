		...
		<form onSubmit={handleSubmit} className={classes.form}>
			<FormControl>
				...
				<Select labelId="house-type-label" id="type" value={values.type || houseTypes[0]} onChange={handleChange}>
					{houseTypes.map((typeH: string) => (
						<MenuItem value={typeH} key={`item-${typeH}`}>{t(`house:types.${typeH}`)}</MenuItem>
					))}
				</Select>
			</FormControl>
			...
			<FormControl error>
				<TextField
					required
					id="title"
					label={t('house:form.general.title')}
					value={values.title}
			...
			<TextareaAutosize
				...
				aria-label="Comment"
				rowsMin={5}
				placeholder={t('house:form.general.comment')}
			...
			<Box display="flex" flexDirection="row" p={1}>
				<Box width="50%" p={1}>
					<FormControlLabel
						control={<Switch checked={values.loaned} onChange={handleChange} name="loaned" />}
						label={t('house:form.general.loaned')}
					/>
				</Box>
				<Box width="50%" p={1}>
					<FormControlLabel
						control={<Switch checked={values.furnished} onChange={handleChange} name="furnished" />}
						label={t('house:form.general.furnished')}
					/>
				</Box>
			</Box>

			<FormControlLabel
				control={<Switch checked={values.active} onChange={handleChange} name="active" />}
				label={t('house:form.general.active')}
			/>
